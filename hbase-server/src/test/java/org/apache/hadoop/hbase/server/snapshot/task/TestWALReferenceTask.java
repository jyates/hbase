...
public class TestWALReferenceTask {
  ...
  public void testRun() throws IOException {
    ...
    FSUtils.touch(fs, log1_1);
    FSUtils.touch(fs, log1_2);
    FSUtils.touch(fs, log2_1);
    FSUtils.touch(fs, log2_2);

    FSUtils.logFileSystemState(fs, testDir, LOG);
    FSUtils.setRootDir(conf, testDir);
    SnapshotDescription snapshot = SnapshotDescription.newBuilder()
        .setName("testWALReferenceSnapshot").build();
    SnapshotErrorListener listener = Mockito.mock(SnapshotErrorListener.class);
