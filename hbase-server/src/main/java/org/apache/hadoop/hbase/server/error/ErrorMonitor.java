 ...
 * @param <E> Type of error that this monitor throws if it finds an error
 * @see ErrorListenable
 */
public interface ErrorMonitor<E extends Exception> extends ErrorListener {

  /**
   * Checks to see if any process to which the monitor is bound has created an error that would
   * cause a failure.
   ...
   * monitor is bound has created an error that would cause a failure.
