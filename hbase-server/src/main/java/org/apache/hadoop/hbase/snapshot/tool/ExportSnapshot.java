...
public final class ExportSnapshot extends Configured implements Tool {
   ...
   */
  private List<List<Path>> getBalancedSplits(final List<Pair<Path, Long>> files, int ngroups) {
  ...
  private Path getInputFolderPath(Configuration conf) throws IOException, InterruptedException {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (outputFs.exists(outputSnapshotDir)) {
      System.err.println("The snapshot '" + Bytes.toString(snapshotName) +
    ...
    if (outputFs.exists(snapshotTmpDir)) {
      System.err.println("A snapshot with the same name '" + Bytes.toString(snapshotName) + "' is in-progress");
      return 1;
    }
