...
public class TakeSnapshotUtils {
   ...
   *         passed in
   */
  public static List<Path> getFamilySnapshotDirectories(SnapshotDescription snapshot, Path snapshotRegionDir,
 FileStatus[] families) {
  ...
  public static void verifyAllLogsGotReferenced(FileSystem fs, Path logsDir,
    ...
    assertEquals("Not the same number of snapshot and original server logs directories",
  ...
  public static void verifyRecoveredEdits(FileSystem fs, Path rootDir, HRegionInfo regionInfo,
    ...
    if (edits == null) {
      assertNull("Snapshot has edits but table doesn't", snapshotEdits);
      return;
    }

    assertNotNull("Table has edits, but snapshot doesn't", snapshotEdits);

    // check each of the files
    assertEquals("Not same number of edits in snapshot as table", edits.length,
    ...
    for (FileStatus edit : edits) {
      for (FileStatus sEdit : snapshotEdits) {
        if (sEdit.getPath().equals(edit.getPath())) {
          assertEquals("Snapshot file" + sEdit.getPath() + " length not equal to the original: "
              + edit.getPath(), edit.getLen(), sEdit.getLen());
          break;
        }
      }
      assertTrue("No edit in snapshot with name:" + edit.getPath(), false);
    }
  }
   ...
   * @throws CorruptedSnapshotException thrown if the two elements don't match
   */
  private static void assertEquals(String msg, long expected, long gotten)
      throws CorruptedSnapshotException {
    if (expected != gotten) {
      throw new CorruptedSnapshotException(msg + ". Expected:" + expected + ", got:" + gotten);
    }
