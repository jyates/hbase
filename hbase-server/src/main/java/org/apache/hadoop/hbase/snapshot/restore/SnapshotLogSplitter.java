 ...
 * those must be splitted (each log contains multiple tables and regions)
 * and must be placed in the region/recovered.edits folder.
 * (recovered.edits files will be played on region startup)
 *
 * In case of Restore: the log can just be splitted in the recovered.edits folder.
...
class SnapshotLogSplitter {
   ...
   */
  private void splitLog(final Path logPath) throws IOException {
